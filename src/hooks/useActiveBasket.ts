...
export interface UseActiveBasketOptions {
  /** Quando false, não carrega todo o catálogo (use paginação server-side). */
  loadProducts?: boolean;
}

export function useActiveBasket(storeId?: string, options?: UseActiveBasketOptions) {
  const loadProducts = options?.loadProducts ?? true;

  return useQuery<ActiveBasket | null>({
    queryKey: ["active-basket", storeId, loadProducts],
    queryFn: async () => {
